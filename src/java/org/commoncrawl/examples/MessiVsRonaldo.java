 ...
 */
public class MessiVsRonaldo extends Configured implements Tool {

  private static final Logger LOG = Logger.getLogger(MessiVsRonaldo.class);

  /**
   * Perform a simple word count mapping on text data from the Common Crawl corpus.
   */
  public static class MessiVsRonaldoMapper
    ...
    public void map(Text key, Text value, OutputCollector<Text, LongWritable> output, Reporter reporter)
      ...
      try {
        ...
        for (String word : pageText.split(" ")) {
          if (word.toLowerCase().equals("messi") || word.toLowerCase().equals("ronaldo"))
  ...
  //String inputPath = "s3n://aws-publicdatasets/common-crawl/parse-output/segment/*/textData-*";
    ...
    job.setMapperClass(MessiVsRonaldo.MessiVsRonaldoMapper.class);
  ...
  /**
   * Main entry point that uses the {@link ToolRunner} class to run the MessiVsRonaldo
   * Hadoop job.
   */
  public static void main(String[] args)
      throws Exception {
    int res = ToolRunner.run(new Configuration(), new MessiVsRonaldo(), args);
    System.exit(res);
  }
}
